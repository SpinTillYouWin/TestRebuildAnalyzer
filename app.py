...
# Line 1: def format_spins_as_html(spins, num_to_show, show_trends=True):
def format_spins_as_html(spins, num_to_show, show_trends=True):
    ...
    if show_trends:
        for i in range(len(spin_list) - 2):
            # Ensure we have a sequence of exactly 3 spins to compare
            if i + 2 >= len(spin_list):  # Skip if we don't have 3 spins left
                break
            # Check for consecutive colors
            color1 = colors.get(spin_list[i], "")
            color2 = colors.get(spin_list[i+1], "")
            color3 = colors.get(spin_list[i+2], "")
            if color1 and color1 == color2 == color3:
                color_name = color1.capitalize()
            ...
            try:
                ...
                continue  # Skip if any spin can't be converted to an integer
...
def render_sides_of_zero_display():
    ...
    """
# Line 1: def validate_spins_input(spins_input):
def validate_spins_input(spins_input, num_to_show=36, show_trends=True):
    ...
    # CHANGED: Use num_to_show and show_trends in format_spins_as_html
    spins_display_value = ", ".join(valid_spins)
    formatted_html = format_spins_as_html(spins_display_value, num_to_show, show_trends)
...
# Line 1: def add_spin(number, current_spins, num_to_show):
def add_spin(number, current_spins, num_to_show, show_trends=True):
    import gradio as gr
    import time
    start_time = time.time()  # CHANGED: Added for performance logging
    
    # CHANGED: Enhanced logging with input details
    print(f"add_spin: Processing number='{number}', current_spins='{current_spins}', num_to_show={num_to_show}, show_trends={show_trends}")
    ...
    # CHANGED: Reuse validate_spins_input for validation, passing num_to_show and show_trends
    spins_input = ", ".join(unique_numbers)
    spins_display_value, formatted_html = validate_spins_input(spins_input, num_to_show, show_trends)
...
# ... (Previous section: undo_last_spin)

def clear_spins(show_trends=True):
    state.last_spins = []
    state.selected_numbers.clear()
    formatted_html = format_spins_as_html("", last_spin_count.default_value, show_trends)
    return "", "", "Spins cleared successfully!", formatted_html, update_spin_counter(), render_sides_of_zero_display()
...
with gr.Blocks(title="WheelPulse by S.T.Y.W 📈") as demo:
    ...
    try:
        spins_textbox.change(
            ...
            inputs=[spins_textbox, last_spin_count_state, show_trends_checkbox, refresh_trigger],
            outputs=[[spins_display, last_spin_display], refresh_trigger],
        ).then(
            fn=analyze_spins,
    ...
    try:
        spins_display.change(
            ...
            inputs=[spins_display, last_spin_count_state, show_trends_checkbox, refresh_trigger],
            outputs=[last_spin_display, refresh_trigger],
        ).then(
            fn=summarize_spin_traits,
            inputs=[last_spin_count_state],
            outputs=[traits_display]
        ).then(
            fn=calculate_hit_percentages,
            inputs=[last_spin_count_state],
    ...
    # ... (Previous handler: spins_display.change)
    ...
    # ... (Next handler: clear_all_button.click)
...
# Line 1: Slider change handler (updated)
    ...
    # ... (Previous handler: load_input.change)
    ...
    # ... (Previous handler: clear_cold_button.click)
    ...
    # ... (Next handler: spins_textbox.change)
    ...
    def toggle_labouchere(progression):
        """Toggle visibility of the Labouchere sequence textbox based on the selected progression."""
