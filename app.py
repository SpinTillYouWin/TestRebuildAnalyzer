...
def precompute_number_to_bets():
    """Precompute a mapping of each number (0-36) to the bets it belongs to across all categories."""
    number_to_bets = {num: {
        "even_money": [],
        "dozens": [],
        "columns": [],
    ...
    for num in range(37):
        for name, numbers in EVEN_MONEY.items():
            if num in numbers:
                number_to_bets[num]["even_money"].append(name)

    # Dozens
    for num in range(37):
        for name, numbers in DOZENS.items():
            if num in numbers:
                number_to_bets[num]["dozens"].append(name)

    # Columns
    for num in range(37):
        for name, numbers in COLUMNS.items():
            if num in numbers:
                number_to_bets[num]["columns"].append(name)

    # Sides of Zero
    for num in range(37):
        if num in LEFT_OF_ZERO_EUROPEAN:
            number_to_bets[num]["sides"].append("Left of Zero")
        if num in RIGHT_OF_ZERO_EUROPEAN:
            number_to_bets[num]["sides"].append("Right of Zero")

    # Streets (e.g., 1-3, 4-6, ..., 34-36)
    for start in range(1, 37, 3):
        street_name = f"Street {start}-{start+2}"
        for num in range(start, start + 3):
            if num <= 36:  # Ensure we don't exceed 36
                number_to_bets[num]["streets"].append(street_name)

    # Double Streets (e.g., 1-6, 4-9, ..., 31-36)
    for start in range(1, 34, 3):  # Step by 3 to create overlapping double streets
        double_street_name = f"Double Street {start}-{start+5}"
        for num in range(start, start + 6):
            if num <= 36:
                number_to_bets[num]["six_lines"].append(double_street_name)

    # Corners (e.g., 1-2-4-5, 2-3-5-6, ..., 32-33-35-36)
    for row in range(1, 12):  # Rows 1 to 11 (since 12 would start at 34)
        for col in range(1, 3):  # Columns 1-2, 2-3
            num1 = (row - 1) * 3 + col  # e.g., row 1, col 1 → 1
            num2 = num1 + 1              # e.g., 2
            num3 = num1 + 3              # e.g., 4
            num4 = num3 + 1              # e.g., 5
            if num4 <= 36:  # Ensure we don't exceed 36
                corner_name = f"Corner {num1}-{num2}-{num3}-{num4}"
                for num in [num1, num2, num3, num4]:
                    number_to_bets[num]["corners"].append(corner_name)

    # Splits (Horizontal: 1-2, 2-3, ..., 35-36; Vertical: 1-4, 4-7, ..., 33-36)
    for num in range(1, 37):
        # Horizontal Splits
        if num % 3 != 0:  # Not the last number in a row (e.g., 3, 6, 9, ...)
            split_name = f"Split {num}-{num+1}"
            number_to_bets[num]["splits"].append(split_name)
            if num + 1 <= 36:
                number_to_bets[num + 1]["splits"].append(split_name)
        # Vertical Splits
        if num <= 33:  # Can have a number below it
            split_name = f"Split {num}-{num+3}"
            number_to_bets[num]["splits"].append(split_name)
            number_to_bets[num + 3]["splits"].append(split_name)

    # Neighbors (using NEIGHBORS_EUROPEAN)
    for num in range(37):
        if num in NEIGHBORS_EUROPEAN:
            ...
            number_to_bets[num]["neighbors"] = neighbors

    return number_to_bets
